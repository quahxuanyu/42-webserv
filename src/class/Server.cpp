...
// void process_connections(std::vector<pollfd> pfds, std::set<int> listening_fds, int fd_count)
// {
// 	for (int i = 0; i < fd_count; i++)
// 	{
// 		int fd = (pfds)[i].fd;
// 		if (listening_fds.count(fd) && (pfds)[i].revents & POLLIN)
...
// 	{
// 		pollfd listener;
// 		listener.fd = listening_sockets[i].sockfd;
// 		listener.events = POLLIN;
// 		pfds.push_back(listener);
// 		listening_fds.insert(listener.fd);
// 	}
// 	int fd_count = pfds.size();
...
// 		process_connections(pfds, listening_fds, fd_count);
// 	}
// }
